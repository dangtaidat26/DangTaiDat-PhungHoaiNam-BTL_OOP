...
public class GameStage {
    private final static int SCREEN_HEIGHT = 720; // chiều rộng màn hình chơi
    private final static int SCREEN_WIDTH = 1200; // chiều dài màn hình chơi
    private final static int SCREEN_TITLEMAP = 30; // độ lớn mỗi ô trong map
    private final static String GAME_TITLE = "Tower Defense";
    private static int[][] MapTitle = new int[24][40]; // mảng số nguyên lưu bản đồ: 0 là đường đi, 1 là cỏ, 2 là viền ranh giới
    private static Image[][] imageMap = new Image[24][40]; // mảng ảnh bản đồ trong game
    ...
    private sample.SniperTower listTower = new sample.SniperTower(); // danh sách các tháp
    private Scanner input = new Scanner(new File("src/MapGame.txt"));
    static  int i = 0;
    static  int j = 0;
    ...
    /* Ánh xạ ảnh tương ứng vào vẽ bản đồ */
    ...
    {
        Image tilemap0 = new Image("file:src/Image/Gach_duong_di.png", 30, 30, true, true);
        Image tilemap1 = new Image("file:src/Image/Gach_co_xanh.png", 30, 30, true, true);
        Image tilemap2 = new Image("file:src/Image/Gach_da.png", 30, 30, true, true);
        /* load vào bản đồ dạng số */
        for (int i = 0; i < 24; i++)
        {
            for (int j = 0; j < 40; j++)
            {
